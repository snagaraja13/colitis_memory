...
plotTopicsUMAP <- function(U1, # UMAP coordinates for first dataset, cells x UMAP1/2
			...
			palette="solar_extra", # BuenColors palette to use
  ...
  for (topic in topicList){
    ...
    g1 <- ggplot(shuf(u1),aes(x=UMAP1,y=UMAP2,color=Topic)) + geom_point(size=ptSize,stroke=0) + theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank()) +
      scale_color_gradientn(colors=jdb_palette(palette)) +
      labs(color=topic)
    if (!is.null(U2)){
      g2 <- ggplot(shuf(u2),aes(x=UMAP1,y=UMAP2,color=Topic)) + geom_point(size=ptSize,stroke=0) + theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank()) +
        scale_color_gradientn(colors=jdb_palette(palette)) +
        labs(color=topic)
      if (!is.null(U3)){
        g3 <- ggplot(shuf(u3),aes(x=UMAP1,y=UMAP2,color=Topic)) + geom_point(size=ptSize,stroke=0) + theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank()) +
          scale_color_gradientn(colors=jdb_palette(palette)) +
          labs(color=topic)
